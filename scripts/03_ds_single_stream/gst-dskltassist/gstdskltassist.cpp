...
GST_DEBUG_CATEGORY_STATIC (gst_dskltassist_debug);
#define GST_CAT_DEFAULT gst_dskltassist_debug
...
#define DEFAULT_PROXY_TTL_FRAMES 3
#define DEFAULT_MAX_CENTER_SHIFT_PX 24.0
#define DEFAULT_MIN_GOOD_POINTS 6
#define DEFAULT_FEATURE_MAX_CORNERS 40
#define DEFAULT_LK_WIN_SIZE 21
#define DEFAULT_PERSON_CLASS_ID 0

#define GST_CAPS_FEATURE_MEMORY_NVMM "memory:NVMM"

static GstStaticPadTemplate gst_dskltassist_sink_template =
...
static GstStaticPadTemplate gst_dskltassist_src_template =
...
#define gst_dskltassist_parent_class parent_class
G_DEFINE_TYPE (GstDsKltAssist, gst_dskltassist, GST_TYPE_BASE_TRANSFORM);

static void gst_dskltassist_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_dskltassist_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static gboolean gst_dskltassist_set_caps (GstBaseTransform * btrans,
    GstCaps * incaps, GstCaps * outcaps);
static gboolean gst_dskltassist_start (GstBaseTransform * btrans);
static gboolean gst_dskltassist_stop (GstBaseTransform * btrans);
static GstFlowReturn gst_dskltassist_transform_ip (GstBaseTransform * btrans,
    GstBuffer * inbuf);
...
clamp_rect (const cv::Rect2f &rect, gint width, gint height)
{
  ...
  float w = std::min (std::max (1.0f, rect.width), max_w);
  float h = std::min (std::max (1.0f, rect.height), max_h);
...
static inline cv::Rect2f
original_to_proc_rect (GstDsKltAssist *self, const NvOSD_RectParams &rect)
{
...
static inline cv::Rect2f
proc_to_original_rect (GstDsKltAssist *self, const cv::Rect2f &rect)
{
...
static inline float
proc_shift_limit (GstDsKltAssist *self)
{
...
static bool
init_features_for_box (const cv::Mat &gray, const cv::Rect2f &bbox,
    guint feature_max_corners, std::vector<cv::Point2f> &features)
{
  if (gray.empty ())
    return false;

  cv::Rect2f expanded (bbox.x - bbox.width * 0.05f, bbox.y - bbox.height * 0.05f,
      bbox.width * 1.10f, bbox.height * 1.10f);
  cv::Rect roi = to_roi (expanded, gray.cols, gray.rows);
...
static void
write_sidecar_row (GstDsKltAssist *self, NvDsFrameMeta *frame_meta,
    guint64 track_id, gboolean proxy_active, guint proxy_age,
    const cv::Rect2f &bbox_proc, const std::string &event_name)
{
...
static void
add_proxy_rect (GstDsKltAssist *self, NvDsBatchMeta *batch_meta,
    NvDsFrameMeta *frame_meta, guint64 track_id, guint proxy_age,
    const cv::Rect2f &bbox_proc)
{
...
convert_frame_to_gray (GstDsKltAssist *self, NvBufSurface *input_buf, guint batch_id,
    cv::Mat &out_gray)
{
...
static bool
advance_proxy (GstDsKltAssist *self, const cv::Mat &prev_gray,
    const cv::Mat &current_gray, KltTrackState &state,
    cv::Rect2f &out_bbox_proc, std::vector<cv::Point2f> &out_features,
    guint &out_good_points, std::string &failure_reason)
{
  std::vector<cv::Point2f> seed_features = state.features;
  if (seed_features.size () < self->min_good_points &&
      !init_features_for_box (prev_gray, state.last_bbox_proc,
          self->feature_max_corners, seed_features)) {
    failure_reason = "no_features";
    return false;
  }
  ...
  cv::calcOpticalFlowPyrLK (prev_gray, current_gray, seed_features, next_features,
  ...
  for (guint i = 0; i < status.size (); ++i) {
    if (!status[i])
      continue;
    const cv::Point2f &pt = next_features[i];
    if (pt.x < 0.0f || pt.y < 0.0f || pt.x >= current_gray.cols || pt.y >= current_gray.rows)
      continue;
    prev_good.push_back (seed_features[i]);
  ...
  out_features = next_good;
  return true;
}

static void
gst_dskltassist_class_init (GstDsKltAssistClass *klass)
{
  ...
  gobject_class->set_property = GST_DEBUG_FUNCPTR (gst_dskltassist_set_property);
  gobject_class->get_property = GST_DEBUG_FUNCPTR (gst_dskltassist_get_property);

  gstbasetransform_class->set_caps = GST_DEBUG_FUNCPTR (gst_dskltassist_set_caps);
  gstbasetransform_class->start = GST_DEBUG_FUNCPTR (gst_dskltassist_start);
  gstbasetransform_class->stop = GST_DEBUG_FUNCPTR (gst_dskltassist_stop);
  gstbasetransform_class->transform_ip =
      GST_DEBUG_FUNCPTR (gst_dskltassist_transform_ip);
  ...
  g_object_class_install_property (gobject_class, PROP_PROCESSING_WIDTH,
      g_param_spec_int ("processing-width", "Processing Width",
          "Width of the frame used for LK assist", 1, G_MAXINT,
  ...
  g_object_class_install_property (gobject_class, PROP_PROCESSING_HEIGHT,
      g_param_spec_int ("processing-height", "Processing Height",
          "Height of the frame used for LK assist", 1, G_MAXINT,
  ...
  g_object_class_install_property (gobject_class, PROP_PROCESS_FULL_FRAME,
      g_param_spec_boolean ("full-frame", "Full Frame",
          "Required for the LK assist path", DEFAULT_PROCESS_FULL_FRAME,
          (GParamFlags) (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
  ...
  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&gst_dskltassist_src_template));
  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&gst_dskltassist_sink_template));

  gst_element_class_set_details_simple (gstelement_class,
      "DsKltAssist plugin",
      "DsKltAssist Plugin",
      "Short-lived KLT proxy assist for DeepStream tracker misses",
      "AID");
}

static void
gst_dskltassist_init (GstDsKltAssist *self)
{
...
static void
gst_dskltassist_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstDsKltAssist *self = GST_DSKLTASSIST (object);
  switch (prop_id) {
    case PROP_UNIQUE_ID:
...
static void
gst_dskltassist_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstDsKltAssist *self = GST_DSKLTASSIST (object);
  switch (prop_id) {
...
static gboolean
gst_dskltassist_start (GstBaseTransform * btrans)
{
  GstDsKltAssist *self = GST_DSKLTASSIST (btrans);

  self->proxy_ttl_frames = read_env_uint ("AID_DSKLTASSIST_PROXY_TTL_FRAMES",
      DEFAULT_PROXY_TTL_FRAMES, 1);
  self->max_center_shift_px = read_env_double ("AID_DSKLTASSIST_MAX_CENTER_SHIFT_PX",
      DEFAULT_MAX_CENTER_SHIFT_PX, 1.0);
  self->min_good_points = read_env_uint ("AID_DSKLTASSIST_MIN_GOOD_POINTS",
      DEFAULT_MIN_GOOD_POINTS, 1);
  self->feature_max_corners = read_env_uint ("AID_DSKLTASSIST_FEATURE_MAX_CORNERS",
      DEFAULT_FEATURE_MAX_CORNERS, 1);
  self->lk_win_size = read_env_uint ("AID_DSKLTASSIST_LK_WIN_SIZE",
      DEFAULT_LK_WIN_SIZE, 3);
  self->person_class_id = read_env_int ("AID_DSKLTASSIST_PERSON_CLASS_ID",
      DEFAULT_PERSON_CLASS_ID, 0);

  const gchar *sidecar_env = g_getenv ("AID_DSKLTASSIST_SIDECAR_PATH");
  if (sidecar_env && *sidecar_env)
    self->sidecar_path = g_strdup (sidecar_env);

  if (!self->track_states)
    self->track_states = new std::unordered_map<std::string, KltTrackState> ();
  if (!self->prev_gray)
  ...
  if (self->sidecar_path) {
    ...
    if (self->sidecar_stream->is_open ()) {
      (*self->sidecar_stream)
          << "frame_num,source_id,track_id,proxy_active,proxy_age,event,left,top,width,height\n";
      self->sidecar_stream->flush ();
    } else {
...
static gboolean
gst_dskltassist_stop (GstBaseTransform * btrans)
{
  GstDsKltAssist *self = GST_DSKLTASSIST (btrans);
  if (self->track_states) {
...
static gboolean
gst_dskltassist_set_caps (GstBaseTransform * btrans, GstCaps * incaps,
    GstCaps * outcaps)
{
  GstDsKltAssist *self = GST_DSKLTASSIST (btrans);
...
static GstFlowReturn
gst_dskltassist_transform_ip (GstBaseTransform * btrans, GstBuffer * inbuf)
{
  GstDsKltAssist *self = GST_DSKLTASSIST (btrans);
  ...
  for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
    ...
    for (NvDsMetaList *l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      KltTrackState &state = (*self->track_states)[key];
      ...
      init_features_for_box (current_gray, bbox_proc, self->feature_max_corners, state.features);
    }

    const float max_shift_proc = proc_shift_limit (self);
    for (auto it = self->track_states->begin (); it != self->track_states->end ();) {
      const std::string &key = it->first;
      KltTrackState &state = it->second;
      ...
      if (miss_frames == 0 || miss_frames > self->proxy_ttl_frames) {
        if (state.proxy_active)
          write_sidecar_row (self, frame_meta, state.track_id, FALSE,
              state.proxy_age_frames, state.last_bbox_proc, "proxy_stop_expired");
      ...
      for (const cv::Rect2f &real_box : real_boxes_proc) {
        if (is_plausible_handoff (real_box, state.last_bbox_proc, max_shift_proc)) {
          write_sidecar_row (self, frame_meta, state.track_id, FALSE,
              state.proxy_age_frames, state.last_bbox_proc, "handoff_real");
          it = self->track_states->erase (it);
          handed_off = true;
          break;
        }
      ...
      if (!advance_proxy (self, *self->prev_gray, current_gray, state,
              next_bbox_proc, next_features, good_points, failure_reason)) {
        write_sidecar_row (self, frame_meta, state.track_id, FALSE,
            state.proxy_age_frames, state.last_bbox_proc, "proxy_stop_" + failure_reason);
        it = self->track_states->erase (it);
        continue;
      }
      ...
      for (const cv::Rect2f &real_box : real_boxes_proc) {
        if (is_plausible_handoff (real_box, next_bbox_proc, max_shift_proc)) {
          write_sidecar_row (self, frame_meta, state.track_id, FALSE,
              state.proxy_age_frames, next_bbox_proc, "handoff_real");
          it = self->track_states->erase (it);
      ...
      add_proxy_rect (self, batch_meta, frame_meta, state.track_id,
          state.proxy_age_frames, state.last_bbox_proc);
      write_sidecar_row (self, frame_meta, state.track_id, TRUE,
          state.proxy_age_frames, state.last_bbox_proc, "proxy_active");
      ++it;
    }
...
static gboolean
dskltassist_plugin_init (GstPlugin * plugin)
{
  GST_DEBUG_CATEGORY_INIT (gst_dskltassist_debug, "dskltassist", 0,
      "dskltassist plugin");

  if (!gst_element_register (plugin, "dsexample", GST_RANK_PRIMARY,
          GST_TYPE_DSKLTASSIST))
    return FALSE;

  gst_element_register (plugin, "dskltassist", GST_RANK_NONE,
      GST_TYPE_DSKLTASSIST);
  return TRUE;
}
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    nvdsgst_dskltassist,
    DESCRIPTION, dskltassist_plugin_init, "8.0", LICENSE, BINARY_PACKAGE, URL)
