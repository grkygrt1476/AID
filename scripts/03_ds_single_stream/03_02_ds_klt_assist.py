...
DEFAULT_TEMPLATE = PROJECT_ROOT / "configs" / "deepstream" / "ds_yolo11_tracker_nvdcf_kltassist.txt"
DEFAULT_PLUGIN_LIB = PROJECT_ROOT / "scripts" / "03_ds_single_stream" / "gst-dskltassist" / "libnvdsgst_dskltassist.so"
...
def parse_args():
    ...
    parser.add_argument("--proxy_ttl_frames", type=int, default=3, help="Max missing-frame bridge length.")
    parser.add_argument(
        "--max_center_shift_px",
        type=float,
        default=24.0,
    ...
    parser.add_argument("--min_good_points", type=int, default=6, help="Minimum LK points required to keep a proxy alive.")
    parser.add_argument("--feature_max_corners", type=int, default=40, help="Max corners seeded per real track.")
...
def build_runtime_env(base_env: dict[str, str], plugin_dir: Path, sidecar_path: Path, args) -> dict[str, str]:
    ...
    env["AID_DSKLTASSIST_PROXY_TTL_FRAMES"] = str(args.proxy_ttl_frames)
    env["AID_DSKLTASSIST_MAX_CENTER_SHIFT_PX"] = str(args.max_center_shift_px)
    env["AID_DSKLTASSIST_MIN_GOOD_POINTS"] = str(args.min_good_points)
    env["AID_DSKLTASSIST_FEATURE_MAX_CORNERS"] = str(args.feature_max_corners)
    env["AID_DSKLTASSIST_LK_WIN_SIZE"] = str(args.lk_win_size)
    env["AID_DSKLTASSIST_SIDECAR_PATH"] = str(sidecar_path)
    return env
...
def main() -> None:
    ...
    if args.no_outputs:
        raise SystemExit("--no_outputs is not supported for this KLT assist wrapper.")
    ...
    validate_file_exists(template_path, "DeepStream assist config template")
    ...
    if not args.dry_run:
        validate_file_exists(plugin_lib, "KLT assist plugin library")

    preflight_rendered_text, refs = render_app_config(
        template_path=template_path,
        input_uri=input_video.as_uri(),
        output_file=Path("/tmp/ds_kltassist_preflight.mp4"),
    )
    ...
    if args.dry_run and not plugin_lib.exists():
        logger.warning("assist plugin library not found; continuing because --dry_run was requested: %s", plugin_lib)

    output_video = run.out_dir / f"{args.out_base}_nvdcf_kltassist.mp4"
    sidecar_path = run.out_dir / "klt_proxy_sidecar.csv"
    ...
    if run.outputs_enabled:
        dump_run_meta(
            run.out_dir,
            {
                ...
                "assist_env": {
                    "AID_DSKLTASSIST_PROXY_TTL_FRAMES": args.proxy_ttl_frames,
                    "AID_DSKLTASSIST_MAX_CENTER_SHIFT_PX": args.max_center_shift_px,
                    "AID_DSKLTASSIST_MIN_GOOD_POINTS": args.min_good_points,
                    "AID_DSKLTASSIST_FEATURE_MAX_CORNERS": args.feature_max_corners,
                    "AID_DSKLTASSIST_LK_WIN_SIZE": args.lk_win_size,
                    "AID_DSKLTASSIST_SIDECAR_PATH": str(sidecar_path),
    ...
    logger.info(
        ...
        "feature_max_corners=%s lk_win_size=%s",
