...
#include "gstdsposeanchorassist.h"
...
GST_DEBUG_CATEGORY_STATIC (gst_dsposeanchorassist_debug);
#define GST_CAT_DEFAULT gst_dsposeanchorassist_debug
...
#define DEFAULT_PATCH_WIDTH_RATIO 0.55
#define DEFAULT_PATCH_HEIGHT_RATIO 0.48
#define DEFAULT_PATCH_Y_OFFSET_RATIO 0.08
#define DEFAULT_HOLD_AFTER_FAIL_FRAMES 3
#define DEFAULT_MIN_PATCH_SIZE_PX 20
#define DEFAULT_PERSON_CLASS_ID 0
#define DEFAULT_POSE_SGIE_UID 2
#define DEFAULT_POSE_MIN_KEYPOINT_CONF 0.20

#define GST_CAPS_FEATURE_MEMORY_NVMM "memory:NVMM"
#define BODYPOSE_KEYPOINT_COUNT 34
#define BODYPOSE_STRIDE 4
#define BODYPOSE_INPUT_W 192.0f
#define BODYPOSE_INPUT_H 256.0f

struct PoseAnchorEstimate
{
  cv::Point2f anchor_proc;
  float shoulder_width_proc;
  float upper_body_height_proc;
  std::string anchor_source;
};

static GstStaticPadTemplate gst_dsposeanchorassist_sink_template =
...
static GstStaticPadTemplate gst_dsposeanchorassist_src_template =
...
#define gst_dsposeanchorassist_parent_class parent_class
G_DEFINE_TYPE (GstDsPoseAnchorAssist, gst_dsposeanchorassist, GST_TYPE_BASE_TRANSFORM);

static void gst_dsposeanchorassist_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_dsposeanchorassist_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static gboolean gst_dsposeanchorassist_set_caps (GstBaseTransform * btrans,
    GstCaps * incaps, GstCaps * outcaps);
static gboolean gst_dsposeanchorassist_start (GstBaseTransform * btrans);
static gboolean gst_dsposeanchorassist_stop (GstBaseTransform * btrans);
static GstFlowReturn gst_dsposeanchorassist_transform_ip (GstBaseTransform * btrans,
    GstBuffer * inbuf);
...
static inline cv::Rect2f
original_to_proc_rect (GstDsPoseAnchorAssist *self, const NvOSD_RectParams &rect)
{
...
static inline cv::Rect2f
proc_to_original_rect (GstDsPoseAnchorAssist *self, const cv::Rect2f &rect)
{
...
static inline float
proc_shift_limit (GstDsPoseAnchorAssist *self)
{
...
static guint64
layer_num_elements (const NvDsInferLayerInfo &layer_info)
{
  guint64 count = 1;
  for (int i = 0; i < layer_info.inferDims.numDims; ++i) {
    const guint64 dim = (layer_info.inferDims.d[i] > 0) ?
        (guint64) layer_info.inferDims.d[i] : 1ULL;
    count *= dim;
  }
  return count;
}
...
static bool
valid_keypoint (const std::vector<cv::Point2f> &points, const std::vector<float> &conf,
    guint idx, float min_conf, cv::Point2f &point_out)
{
  if (idx >= points.size () || idx >= conf.size ())
    return false;
  if (!std::isfinite (points[idx].x) || !std::isfinite (points[idx].y))
    return false;
  if (conf[idx] < min_conf)
    return false;
  point_out = points[idx];
  return true;
}

static bool
mean_valid_keypoints (const std::vector<cv::Point2f> &points, const std::vector<float> &conf,
    const guint *indices, guint num_indices, float min_conf, cv::Point2f &point_out)
{
  cv::Point2f sum (0.0f, 0.0f);
  guint count = 0;
  for (guint i = 0; i < num_indices; ++i) {
    cv::Point2f point;
    if (!valid_keypoint (points, conf, indices[i], min_conf, point))
      continue;
    sum += point;
    ++count;
  }
  ...
  point_out = cv::Point2f (sum.x / count, sum.y / count);
  return true;
}

static bool
extract_pose25d_points (GstDsPoseAnchorAssist *self, NvDsObjectMeta *obj_meta,
    const cv::Rect2f &bbox_proc, std::vector<cv::Point2f> &points_out,
    std::vector<float> &conf_out, std::string &failure_reason)
{
  for (NvDsMetaList *l_user = obj_meta->obj_user_meta_list; l_user != NULL; l_user = l_user->next) {
    ...
    if (!tensor_meta || tensor_meta->unique_id != self->pose_sgie_uid)
      continue;

    for (guint i = 0; i < tensor_meta->num_output_layers; ++i) {
      NvDsInferLayerInfo *layer_info = &tensor_meta->output_layers_info[i];
      if (!layer_info || !layer_info->layerName)
        continue;
      if (std::strcmp (layer_info->layerName, "pose25d") != 0)
        continue;

      float *data = NULL;
      if (tensor_meta->out_buf_ptrs_host)
        data = (float *) tensor_meta->out_buf_ptrs_host[i];
      if (!data)
        data = (float *) layer_info->buffer;
      if (!data) {
        failure_reason = "pose_tensor_no_host_buffer";
        return false;
      }

      const guint64 need_count = BODYPOSE_KEYPOINT_COUNT * BODYPOSE_STRIDE;
      if (layer_num_elements (*layer_info) < need_count) {
        failure_reason = "pose_tensor_shape_mismatch";
        return false;
      }

      points_out.resize (BODYPOSE_KEYPOINT_COUNT);
      conf_out.resize (BODYPOSE_KEYPOINT_COUNT);
      const float scale_x = bbox_proc.width / BODYPOSE_INPUT_W;
      const float scale_y = bbox_proc.height / BODYPOSE_INPUT_H;
      for (guint kp = 0; kp < BODYPOSE_KEYPOINT_COUNT; ++kp) {
        const guint idx = kp * BODYPOSE_STRIDE;
        points_out[kp].x = bbox_proc.x + data[idx] * scale_x;
        points_out[kp].y = bbox_proc.y + data[idx + 1] * scale_y;
        conf_out[kp] = data[idx + 3];
      }
      return true;
    }
  }

  failure_reason = "pose_tensor_missing";
  return false;
}

static bool
resolve_pose_anchor_from_meta (GstDsPoseAnchorAssist *self, NvDsObjectMeta *obj_meta,
    const cv::Rect2f &bbox_proc, PoseAnchorEstimate &estimate,
    std::string &failure_reason)
{
  std::vector<cv::Point2f> keypoints_proc;
  std::vector<float> keypoints_conf;
  if (!extract_pose25d_points (self, obj_meta, bbox_proc, keypoints_proc, keypoints_conf, failure_reason))
    return false;

  const guint head_indices[] = {15, 16, 17, 18, 19};
  const guint hip_indices[] = {1, 2};

  cv::Point2f head_point;
  cv::Point2f left_shoulder;
  cv::Point2f right_shoulder;
  cv::Point2f shoulder_mid;
  cv::Point2f hip_mid;

  const bool has_head =
      mean_valid_keypoints (keypoints_proc, keypoints_conf, head_indices,
          G_N_ELEMENTS (head_indices), (float) self->pose_min_keypoint_conf, head_point);
  const bool has_left_shoulder =
      valid_keypoint (keypoints_proc, keypoints_conf, 20,
          (float) self->pose_min_keypoint_conf, left_shoulder);
  const bool has_right_shoulder =
      valid_keypoint (keypoints_proc, keypoints_conf, 21,
          (float) self->pose_min_keypoint_conf, right_shoulder);
  const bool has_hip_mid =
      mean_valid_keypoints (keypoints_proc, keypoints_conf, hip_indices,
          G_N_ELEMENTS (hip_indices), (float) self->pose_min_keypoint_conf, hip_mid);

  const bool has_shoulder_mid = has_left_shoulder && has_right_shoulder;
  float shoulder_width_proc = 0.0f;
  if (has_shoulder_mid) {
    shoulder_mid = 0.5f * (left_shoulder + right_shoulder);
    const cv::Point2f delta = right_shoulder - left_shoulder;
    shoulder_width_proc = std::sqrt (delta.x * delta.x + delta.y * delta.y);
  }

  cv::Point2f anchor_proc;
  std::string anchor_source = "weak_pose_tensor";
  if (has_head && has_shoulder_mid) {
    anchor_proc = 0.5f * (head_point + shoulder_mid);
    anchor_source = "pose25d_head_shoulder_center";
  } else if (has_shoulder_mid) {
    anchor_proc = shoulder_mid;
    anchor_source = "pose25d_shoulder_mid";
  } else if (has_head && has_left_shoulder) {
    anchor_proc = 0.5f * (head_point + left_shoulder);
    anchor_source = "pose25d_head_left_shoulder";
  } else if (has_head && has_right_shoulder) {
    anchor_proc = 0.5f * (head_point + right_shoulder);
    anchor_source = "pose25d_head_right_shoulder";
  } else if (has_head) {
    anchor_proc = head_point;
    anchor_source = "pose25d_head_only";
  } else if (has_left_shoulder) {
    anchor_proc = left_shoulder;
    anchor_source = "pose25d_left_shoulder";
  } else if (has_right_shoulder) {
    anchor_proc = right_shoulder;
    anchor_source = "pose25d_right_shoulder";
  } else {
    failure_reason = "weak_pose_tensor";
    return false;
  }

  anchor_proc.x = std::clamp (anchor_proc.x, bbox_proc.x, bbox_proc.x + bbox_proc.width);
  anchor_proc.y = std::clamp (anchor_proc.y, bbox_proc.y, bbox_proc.y + bbox_proc.height);

  float upper_body_height_proc = 0.0f;
  if (has_head && has_hip_mid) {
    upper_body_height_proc = std::max (1.0f, hip_mid.y - head_point.y);
  } else if (has_head && has_shoulder_mid) {
    upper_body_height_proc = std::max (1.0f, (shoulder_mid.y - head_point.y) * 2.2f);
  } else if (shoulder_width_proc > 0.0f) {
    upper_body_height_proc = shoulder_width_proc * 2.0f;
  }

  estimate.anchor_proc = anchor_proc;
  estimate.shoulder_width_proc = shoulder_width_proc;
  estimate.upper_body_height_proc = upper_body_height_proc;
  estimate.anchor_source = anchor_source;
  return true;
}

static bool
build_patch_geometry (GstDsPoseAnchorAssist *self, NvDsObjectMeta *obj_meta,
    const cv::Rect2f &bbox_proc, PoseAnchorPatchGeometry &patch,
    std::string &failure_reason)
{
  PoseAnchorEstimate pose_estimate;
  const bool has_true_pose =
      resolve_pose_anchor_from_meta (self, obj_meta, bbox_proc, pose_estimate, failure_reason);

  cv::Point2f anchor_proc;
  float patch_width = std::max ((float) self->min_patch_size_px,
      bbox_proc.width * (float) self->patch_width_ratio);
  float patch_height = std::max ((float) self->min_patch_size_px,
      bbox_proc.height * (float) self->patch_height_ratio);
  std::string patch_source = "pose_unavailable_fallback";
  std::string pose_anchor_source = failure_reason.empty () ? "pose_unavailable_fallback" : failure_reason;

  if (has_true_pose) {
    anchor_proc = pose_estimate.anchor_proc;
    patch_source = "true_pose";
    pose_anchor_source = pose_estimate.anchor_source;
    if (pose_estimate.shoulder_width_proc > 0.0f) {
      patch_width = std::max (patch_width, pose_estimate.shoulder_width_proc * 1.9f);
      patch_height = std::max (patch_height, pose_estimate.shoulder_width_proc * 2.2f);
    }
    if (pose_estimate.upper_body_height_proc > 0.0f)
      patch_height = std::max (patch_height, pose_estimate.upper_body_height_proc * 0.65f);
    patch_width = std::clamp (patch_width, (float) self->min_patch_size_px, bbox_proc.width * 0.95f);
    patch_height = std::clamp (patch_height, (float) self->min_patch_size_px, bbox_proc.height * 0.90f);
    ...
    pose_anchor_source = failure_reason.empty () ? "pose_unavailable_fallback" : failure_reason;
  }

  const float patch_center_y = anchor_proc.y + patch_height * (float) self->patch_y_offset_ratio;
  cv::Rect2f patch_rect (
      anchor_proc.x - patch_width * 0.5f,
      patch_center_y - patch_height * 0.5f,
      patch_width,
      patch_height);
  ...
  patch.patch_source = patch_source;
  patch.pose_anchor_source = pose_anchor_source;
  return true;
}
...
static void
write_sidecar_row (GstDsPoseAnchorAssist *self, NvDsFrameMeta *frame_meta,
    ...
    const std::string &pose_anchor_source, guint tracked_points,
    const cv::Point2f &flow_delta_proc, float flow_mag_proc)
{
...
static void
add_proxy_overlay (GstDsPoseAnchorAssist *self, NvDsBatchMeta *batch_meta,
    ...
    const char *mode, const cv::Rect2f &bbox_proc, const cv::Rect2f &patch_proc)
{
  ...
  if (g_strcmp0 (mode, "frozen_hold") == 0) {
    ...
    bbox_rect.border_color.red = 0.1;
    bbox_rect.border_color.green = 1.0;
    bbox_rect.border_color.blue = 0.1;
  }
  ...
  if (is_valid_rect (patch_proc)) {
    ...
    if (g_strcmp0 (mode, "frozen_hold") == 0) {
      ...
      patch_rect.border_color.green = 0.9;
  ...
  if (g_strcmp0 (mode, "frozen_hold") == 0) {
    text.display_text = g_strdup_printf ("PAA %" G_GUINT64_FORMAT " hold +%u",
        (guint64) track_id, proxy_age);
  } else {
    text.display_text = g_strdup_printf ("PAA %" G_GUINT64_FORMAT " +%u",
        (guint64) track_id, proxy_age);
  }
  ...
  text.font_params.font_color = (NvOSD_ColorParams) {0.6, 1.0, 0.6, 1.0};

  nvds_add_display_meta_to_frame (frame_meta, display_meta);
}

static GstFlowReturn
convert_frame_to_gray (GstDsPoseAnchorAssist *self, NvBufSurface *input_buf,
...
static bool
advance_patch_proxy (GstDsPoseAnchorAssist *self, const cv::Mat &prev_gray,
    const cv::Mat &current_gray, PoseAnchorTrackState &state,
...
static void
gst_dsposeanchorassist_class_init (GstDsPoseAnchorAssistClass *klass)
{
  ...
  gobject_class->set_property = GST_DEBUG_FUNCPTR (gst_dsposeanchorassist_set_property);
  gobject_class->get_property = GST_DEBUG_FUNCPTR (gst_dsposeanchorassist_get_property);

  gstbasetransform_class->set_caps = GST_DEBUG_FUNCPTR (gst_dsposeanchorassist_set_caps);
  gstbasetransform_class->start = GST_DEBUG_FUNCPTR (gst_dsposeanchorassist_start);
  gstbasetransform_class->stop = GST_DEBUG_FUNCPTR (gst_dsposeanchorassist_stop);
  gstbasetransform_class->transform_ip =
      GST_DEBUG_FUNCPTR (gst_dsposeanchorassist_transform_ip);
  ...
  g_object_class_install_property (gobject_class, PROP_PROCESSING_WIDTH,
      g_param_spec_int ("processing-width", "Processing Width",
          "Width of the frame used for pose-anchor assist", 1, G_MAXINT,
  ...
  g_object_class_install_property (gobject_class, PROP_PROCESSING_HEIGHT,
      g_param_spec_int ("processing-height", "Processing Height",
          "Height of the frame used for pose-anchor assist", 1, G_MAXINT,
  ...
  g_object_class_install_property (gobject_class, PROP_PROCESS_FULL_FRAME,
      g_param_spec_boolean ("full-frame", "Full Frame",
          "Required for the pose-anchor assist path", DEFAULT_PROCESS_FULL_FRAME,
          (GParamFlags) (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
  ...
  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&gst_dsposeanchorassist_src_template));
  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&gst_dsposeanchorassist_sink_template));

  gst_element_class_set_details_simple (gstelement_class,
      "DsPoseAnchorAssist plugin",
      "DsPoseAnchorAssist Plugin",
      "Pose-anchor plus KLT continuity assist for DeepStream tracker misses",
      "AID");
}

static void
gst_dsposeanchorassist_init (GstDsPoseAnchorAssist *self)
{
  ...
  self->pose_sgie_uid = DEFAULT_POSE_SGIE_UID;
  self->pose_min_keypoint_conf = DEFAULT_POSE_MIN_KEYPOINT_CONF;
...
static void
gst_dsposeanchorassist_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstDsPoseAnchorAssist *self = GST_DSPOSEANCHORASSIST (object);
  switch (prop_id) {
    case PROP_UNIQUE_ID:
...
static void
gst_dsposeanchorassist_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstDsPoseAnchorAssist *self = GST_DSPOSEANCHORASSIST (object);
  switch (prop_id) {
...
static gboolean
gst_dsposeanchorassist_start (GstBaseTransform * btrans)
{
  GstDsPoseAnchorAssist *self = GST_DSPOSEANCHORASSIST (btrans);

  self->hard_max_proxy_age_frames =
      read_env_uint ("AID_DSPOSEANCHORASSIST_HARD_MAX_PROXY_AGE_FRAMES",
          DEFAULT_HARD_MAX_PROXY_AGE_FRAMES, 1);
  self->max_center_shift_px =
      read_env_double ("AID_DSPOSEANCHORASSIST_MAX_CENTER_SHIFT_PX",
          DEFAULT_MAX_CENTER_SHIFT_PX, 1.0);
  self->min_good_points =
      read_env_uint ("AID_DSPOSEANCHORASSIST_MIN_GOOD_POINTS",
          DEFAULT_MIN_GOOD_POINTS, 1);
  self->feature_max_corners =
      read_env_uint ("AID_DSPOSEANCHORASSIST_FEATURE_MAX_CORNERS",
          DEFAULT_FEATURE_MAX_CORNERS, 1);
  self->lk_win_size =
      read_env_uint ("AID_DSPOSEANCHORASSIST_LK_WIN_SIZE",
          DEFAULT_LK_WIN_SIZE, 3);
  self->patch_width_ratio =
      read_env_double ("AID_DSPOSEANCHORASSIST_PATCH_WIDTH_RATIO",
          DEFAULT_PATCH_WIDTH_RATIO, 0.05);
  self->patch_height_ratio =
      read_env_double ("AID_DSPOSEANCHORASSIST_PATCH_HEIGHT_RATIO",
          DEFAULT_PATCH_HEIGHT_RATIO, 0.05);
  self->patch_y_offset_ratio =
      read_env_double ("AID_DSPOSEANCHORASSIST_PATCH_Y_OFFSET_RATIO",
          DEFAULT_PATCH_Y_OFFSET_RATIO, 0.0);
  self->freeze_on_patch_fail =
      read_env_bool ("AID_DSPOSEANCHORASSIST_FREEZE_ON_PATCH_FAIL", TRUE);
  self->hold_after_fail_frames =
      read_env_uint ("AID_DSPOSEANCHORASSIST_HOLD_AFTER_FAIL_FRAMES",
          DEFAULT_HOLD_AFTER_FAIL_FRAMES, 0);
  self->person_class_id =
      read_env_int ("AID_DSPOSEANCHORASSIST_PERSON_CLASS_ID",
          DEFAULT_PERSON_CLASS_ID, 0);
  self->pose_sgie_uid =
      read_env_uint ("AID_DSPOSEANCHORASSIST_POSE_SGIE_UID",
          DEFAULT_POSE_SGIE_UID, 1);
  self->pose_min_keypoint_conf =
      read_env_double ("AID_DSPOSEANCHORASSIST_POSE_MIN_KEYPOINT_CONF",
          DEFAULT_POSE_MIN_KEYPOINT_CONF, 0.0);

  const gchar *sidecar_env = g_getenv ("AID_DSPOSEANCHORASSIST_SIDECAR_PATH");
  if (sidecar_env && *sidecar_env)
    self->sidecar_path = g_strdup (sidecar_env);

  if (!self->track_states)
    self->track_states = new std::unordered_map<std::string, PoseAnchorTrackState> ();
  if (!self->prev_gray)
  ...
  GST_INFO_OBJECT (self,
      "current baseline has no pose metadata by default; this branch expects pose25d tensor meta from SGIE unique-id=%u and falls back explicitly when unavailable",
      self->pose_sgie_uid);
  return TRUE;
}

static gboolean
gst_dsposeanchorassist_stop (GstBaseTransform * btrans)
{
  GstDsPoseAnchorAssist *self = GST_DSPOSEANCHORASSIST (btrans);
  if (self->track_states) {
...
static gboolean
gst_dsposeanchorassist_set_caps (GstBaseTransform * btrans, GstCaps * incaps,
    GstCaps * outcaps)
{
  GstDsPoseAnchorAssist *self = GST_DSPOSEANCHORASSIST (btrans);
...
static GstFlowReturn
gst_dsposeanchorassist_transform_ip (GstBaseTransform * btrans, GstBuffer * inbuf)
{
  GstDsPoseAnchorAssist *self = GST_DSPOSEANCHORASSIST (btrans);
  ...
  for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
    ...
    for (NvDsMetaList *l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      state.patch_source = patch_ok ? patch_geometry.patch_source : "pose_unavailable_fallback";
      state.pose_anchor_source = patch_ok ? patch_geometry.pose_anchor_source : (patch_reason.empty () ? "pose_unavailable_fallback" : patch_reason);
      state.last_anchor_proc = patch_ok ? patch_geometry.anchor_proc : rect_center (bbox_proc);
      ...
      write_sidecar_row (self, frame_meta, state.track_id, "real", FALSE, 0,
          ...
          state.patch_source, state.pose_anchor_source, state.last_tracked_points,
    ...
    for (auto it = self->track_states->begin (); it != self->track_states->end ();) {
      ...
      PoseAnchorTrackState &state = it->second;
      ...
      if (miss_frames == 0 || miss_frames > self->hard_max_proxy_age_frames) {
        if (state.proxy_active || state.frozen_hold_active) {
          write_sidecar_row (self, frame_meta, state.track_id,
              ...
              state.pose_anchor_source, state.last_tracked_points,
              state.last_flow_delta_proc, state.last_flow_mag_proc);
        }
      ...
      for (const cv::Rect2f &real_box : real_boxes_proc) {
        if (is_plausible_handoff (real_box, next_bbox_proc, max_shift_proc)) {
          write_sidecar_row (self, frame_meta, state.track_id, "proxy", FALSE,
              ...
              state.pose_anchor_source, good_points, flow_delta_proc, flow_mag_proc);
          it = self->track_states->erase (it);
      ...
      state.last_anchor_proc += flow_delta_proc;
...
static gboolean
dsposeanchorassist_plugin_init (GstPlugin * plugin)
{
  GST_DEBUG_CATEGORY_INIT (gst_dsposeanchorassist_debug, "dsposeanchorassist", 0,
      "dsposeanchorassist plugin");

  if (!gst_element_register (plugin, "dsposeanchorassist", GST_RANK_PRIMARY,
          GST_TYPE_DSPOSEANCHORASSIST))
    return FALSE;
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    nvdsgst_dsposeanchorassist,
    DESCRIPTION, dsposeanchorassist_plugin_init, "8.0", LICENSE, BINARY_PACKAGE, URL)
