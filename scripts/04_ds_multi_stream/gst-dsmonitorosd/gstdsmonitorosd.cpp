...
GST_DEBUG_CATEGORY_STATIC (gst_dsmonitorosd_debug);
#define GST_CAT_DEFAULT gst_dsmonitorosd_debug
...
static GstStaticPadTemplate gst_dsmonitorosd_sink_template =
...
static GstStaticPadTemplate gst_dsmonitorosd_src_template =
...
#define gst_dsmonitorosd_parent_class parent_class
G_DEFINE_TYPE (GstDsMonitorOsd, gst_dsmonitorosd, GST_TYPE_BASE_TRANSFORM);

static void gst_dsmonitorosd_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_dsmonitorosd_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static gboolean gst_dsmonitorosd_set_caps (GstBaseTransform * btrans,
    GstCaps * incaps, GstCaps * outcaps);
static gboolean gst_dsmonitorosd_start (GstBaseTransform * btrans);
static gboolean gst_dsmonitorosd_stop (GstBaseTransform * btrans);
static GstFlowReturn gst_dsmonitorosd_transform_ip (GstBaseTransform * btrans,
    GstBuffer * inbuf);

struct MonitorFrameStats
{
...
static bool
parse_roi_poly (const gchar *raw, std::vector<MonitorPoint> &out_points)
{
  out_points.clear ();
  if (!raw || !*raw)
    return true;
...
static void
free_source_configs (GstDsMonitorOsd *self)
{
...
static gboolean
load_source_configs (GstDsMonitorOsd *self)
{
  free_source_configs (self);
  self->source_configs = new std::unordered_map<guint, MonitorSourceConfig> ();

  self->source_count = read_env_uint ("AID_DSMONITOROSD_SOURCE_COUNT",
      DEFAULT_SOURCE_COUNT, 1);
  self->person_class_id = read_env_int ("AID_DSMONITOROSD_PERSON_CLASS_ID",
      DEFAULT_PERSON_CLASS_ID, 0);

  for (guint source_id = 0; source_id < self->source_count; ++source_id) {
    MonitorSourceConfig config = default_source_config (source_id);

    gchar *label_env = g_strdup_printf ("AID_DSMONITOROSD_SOURCE%u_LABEL", source_id);
    gchar *status_env = g_strdup_printf ("AID_DSMONITOROSD_SOURCE%u_ROI_STATUS", source_id);
    gchar *poly_env = g_strdup_printf ("AID_DSMONITOROSD_SOURCE%u_ROI_POLY", source_id);
...
static void
set_fill_rect (NvOSD_RectParams &rect, gint x, gint y, gint width, gint height,
    const NvOSD_ColorParams &bg_color)
{
  rect.left = x;
  rect.top = y;
  ...
  rect.border_color = (NvOSD_ColorParams) {0.0, 0.0, 0.0, 0.0};
}

static gboolean
point_in_polygon (const std::vector<MonitorPoint> &polygon, gdouble x, gdouble y)
{
  if (polygon.size () < 3)
    return FALSE;
...
static gboolean
object_is_candidate (const MonitorSourceConfig &config, const NvDsObjectMeta *obj_meta)
{
...
static MonitorFrameStats
apply_object_styles_and_collect_stats (GstDsMonitorOsd *self, NvDsFrameMeta *frame_meta,
    const MonitorSourceConfig &config)
{
  MonitorFrameStats stats;
...
static const gchar *
frame_status_label (const MonitorSourceConfig &config, const MonitorFrameStats &stats)
{
...
static NvOSD_ColorParams
frame_status_color (const gchar *status)
{
  if (g_strcmp0 (status, "CANDIDATE") == 0)
    return (NvOSD_ColorParams) {1.0, 0.78, 0.32, 1.0};
  if (g_strcmp0 (status, "NO ROI") == 0)
    return (NvOSD_ColorParams) {0.88, 0.90, 0.94, 1.0};
  return (NvOSD_ColorParams) {0.62, 0.96, 0.72, 1.0};
}

static void
add_monitor_display_meta (GstDsMonitorOsd *self, NvDsBatchMeta *batch_meta,
    NvDsFrameMeta *frame_meta)
{
  MonitorSourceConfig config = default_source_config (frame_meta->source_id);
  if (self->source_configs) {
  ...
  const MonitorFrameStats stats = apply_object_styles_and_collect_stats (self, frame_meta, config);
  ...
  if (config.roi_points.size () >= 3) {
    ...
    for (guint i = 0; i < line_count; ++i) {
      const MonitorPoint &p0 = config.roi_points[i];
      const MonitorPoint &p1 = config.roi_points[(i + 1) % config.roi_points.size ()];
...
static void
gst_dsmonitorosd_class_init (GstDsMonitorOsdClass *klass)
{
  ...
  gobject_class->set_property = GST_DEBUG_FUNCPTR (gst_dsmonitorosd_set_property);
  gobject_class->get_property = GST_DEBUG_FUNCPTR (gst_dsmonitorosd_get_property);

  gstbasetransform_class->set_caps = GST_DEBUG_FUNCPTR (gst_dsmonitorosd_set_caps);
  gstbasetransform_class->start = GST_DEBUG_FUNCPTR (gst_dsmonitorosd_start);
  gstbasetransform_class->stop = GST_DEBUG_FUNCPTR (gst_dsmonitorosd_stop);
  gstbasetransform_class->transform_ip =
      GST_DEBUG_FUNCPTR (gst_dsmonitorosd_transform_ip);
  ...
  g_object_class_install_property (gobject_class, PROP_PROCESS_FULL_FRAME,
      g_param_spec_boolean ("full-frame", "Full Frame",
          "Required for the monitoring overlay path", DEFAULT_PROCESS_FULL_FRAME,
          (GParamFlags) (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
  ...
  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&gst_dsmonitorosd_src_template));
  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&gst_dsmonitorosd_sink_template));

  gst_element_class_set_details_simple (gstelement_class,
      "DsMonitorOsd plugin",
      "DsMonitorOsd Plugin",
      "Stage 04 monitoring overlay plugin for DeepStream multistream tiles",
      "AID");
}

static void
gst_dsmonitorosd_init (GstDsMonitorOsd *self)
{
...
static void
gst_dsmonitorosd_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstDsMonitorOsd *self = GST_DSMONITOROSD (object);
  switch (prop_id) {
    case PROP_UNIQUE_ID:
...
static void
gst_dsmonitorosd_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstDsMonitorOsd *self = GST_DSMONITOROSD (object);
  switch (prop_id) {
...
static gboolean
gst_dsmonitorosd_start (GstBaseTransform * btrans)
{
  GstDsMonitorOsd *self = GST_DSMONITOROSD (btrans);
  return load_source_configs (self);
}

static gboolean
gst_dsmonitorosd_stop (GstBaseTransform * btrans)
{
  GstDsMonitorOsd *self = GST_DSMONITOROSD (btrans);
  free_source_configs (self);
  return TRUE;
...
static gboolean
gst_dsmonitorosd_set_caps (GstBaseTransform * btrans, GstCaps * incaps,
    GstCaps * outcaps)
{
  GstDsMonitorOsd *self = GST_DSMONITOROSD (btrans);
...
gst_dsmonitorosd_transform_ip (GstBaseTransform * btrans, GstBuffer * inbuf)
{
  GstDsMonitorOsd *self = GST_DSMONITOROSD (btrans);
...
static gboolean
dsmonitorosd_plugin_init (GstPlugin * plugin)
{
  GST_DEBUG_CATEGORY_INIT (gst_dsmonitorosd_debug, "dsmonitorosd", 0,
      "dsmonitorosd plugin");

  if (!gst_element_register (plugin, "dsexample", GST_RANK_PRIMARY,
          GST_TYPE_DSMONITOROSD))
    return FALSE;

  gst_element_register (plugin, "dsmonitorosd", GST_RANK_NONE,
      GST_TYPE_DSMONITOROSD);
  return TRUE;
}
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    nvdsgst_dsmonitorosd,
    DESCRIPTION, dsmonitorosd_plugin_init, "8.0", LICENSE, BINARY_PACKAGE, URL)
