...
#include <ostream>
#include <fstream>
#include "gstdsholdbox.h"
#include <sys/time.h>
GST_DEBUG_CATEGORY_STATIC (gst_dsholdbox_debug);
#define GST_CAT_DEFAULT gst_dsholdbox_debug
...
static GstStaticPadTemplate gst_dsholdbox_sink_template =
...
static GstStaticPadTemplate gst_dsholdbox_src_template =
...
#define gst_dsholdbox_parent_class parent_class
G_DEFINE_TYPE (GstDsExample, gst_dsholdbox, GST_TYPE_BASE_TRANSFORM);

static void gst_dsholdbox_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_dsholdbox_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);

static gboolean gst_dsholdbox_set_caps (GstBaseTransform * btrans,
    GstCaps * incaps, GstCaps * outcaps);
static gboolean gst_dsholdbox_start (GstBaseTransform * btrans);
static gboolean gst_dsholdbox_stop (GstBaseTransform * btrans);


static inline std::string
make_track_key(guint source_id, guint64 object_id)
{
  return std::to_string(source_id) + ":" + std::to_string((unsigned long)object_id);
}

static void
add_hold_rect(NvDsBatchMeta *batch_meta, NvDsFrameMeta *frame_meta,
              float left, float top, float width, float height)
{
  ...
  NvOSD_RectParams &r = display_meta->rect_params[0];
  ...
  r.border_color.green = 0.5;
  r.border_color.blue = 0.0;
  r.border_color.alpha = 1.0;

  nvds_add_display_meta_to_frame(frame_meta, display_meta);
}

static GstFlowReturn gst_dsholdbox_transform_ip (GstBaseTransform *
 ...
 */
static void
gst_dsholdbox_class_init (GstDsExampleClass * klass)
{
  ...
  gobject_class->set_property = GST_DEBUG_FUNCPTR (gst_dsholdbox_set_property);
  gobject_class->get_property = GST_DEBUG_FUNCPTR (gst_dsholdbox_get_property);

  gstbasetransform_class->set_caps = GST_DEBUG_FUNCPTR (gst_dsholdbox_set_caps);
  gstbasetransform_class->start = GST_DEBUG_FUNCPTR (gst_dsholdbox_start);
  gstbasetransform_class->stop = GST_DEBUG_FUNCPTR (gst_dsholdbox_stop);

  gstbasetransform_class->transform_ip =
      GST_DEBUG_FUNCPTR (gst_dsholdbox_transform_ip);
  ...
  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&gst_dsholdbox_src_template));
  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&gst_dsholdbox_sink_template));
...
static void
gst_dsholdbox_init (GstDsExample * dsexample)
{
  ...
  dsexample->hold_ns = 300 * GST_MSECOND;
  dsexample->hold_cache = NULL;
 ...
 */
static void
gst_dsholdbox_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstDsExample *dsexample = GST_DSHOLDBOX (object);
  switch (prop_id) {
    case PROP_UNIQUE_ID:
 ...
 */
static void
gst_dsholdbox_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstDsExample *dsexample = GST_DSHOLDBOX (object);
 ...
 */
static gboolean
gst_dsholdbox_start (GstBaseTransform * btrans)
{
  GstDsExample *dsexample = GST_DSHOLDBOX (btrans);
  if (!dsexample->hold_cache)
    dsexample->hold_cache = new std::unordered_map<std::string, HoldBoxState>();
 ...
 */
static gboolean
gst_dsholdbox_stop (GstBaseTransform * btrans)
{
  GstDsExample *dsexample = GST_DSHOLDBOX (btrans);
  if (dsexample->hold_cache) {
    delete dsexample->hold_cache;
    dsexample->hold_cache = NULL;
  }
 ...
 */
static gboolean
gst_dsholdbox_set_caps (GstBaseTransform * btrans, GstCaps * incaps,
    GstCaps * outcaps)
{
  GstDsExample *dsexample = GST_DSHOLDBOX (btrans);
 ...
 */
static GstFlowReturn
gst_dsholdbox_transform_ip (GstBaseTransform * btrans, GstBuffer * inbuf)
{
  GstDsExample *dsexample = GST_DSHOLDBOX (btrans);
  ...
  guint64 now_pts = GST_BUFFER_PTS(inbuf);
  if (now_pts == GST_CLOCK_TIME_NONE)
    now_pts = 0;
  ...
  for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
    ...
    for (NvDsMetaList *l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      std::string key = make_track_key(frame_meta->source_id, obj_meta->object_id);
      seen_this_frame[key] = true;

      HoldBoxState hb;
      hb.left = obj_meta->rect_params.left;
      hb.top = obj_meta->rect_params.top;
      hb.width = obj_meta->rect_params.width;
      hb.height = obj_meta->rect_params.height;
      hb.last_seen_pts = now_pts;

      (*dsexample->hold_cache)[key] = hb;
    }

    std::string source_prefix = std::to_string(frame_meta->source_id) + ":";

    for (auto it = dsexample->hold_cache->begin(); it != dsexample->hold_cache->end(); ) {
      const std::string &key = it->first;
      HoldBoxState &hb = it->second;

      if (key.rfind(source_prefix, 0) != 0) {
        ++it;
        continue;
      }
      ...
      if (now_pts > 0 && hb.last_seen_pts > 0 && now_pts >= hb.last_seen_pts) {
        within_hold = ((now_pts - hb.last_seen_pts) <= dsexample->hold_ns);
      }

      if (within_hold) {
        add_hold_rect(batch_meta, frame_meta, hb.left, hb.top, hb.width, hb.height);
        ++it;
      } else {
        it = dsexample->hold_cache->erase(it);
      }
    }
  }
 ...
 */
static gboolean
dsholdbox_plugin_init (GstPlugin * plugin)
{
  GST_DEBUG_CATEGORY_INIT (gst_dsholdbox_debug, "dsholdbox", 0,
      "dsholdbox plugin");
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    nvdsgst_dsholdbox,
    DESCRIPTION, dsholdbox_plugin_init, "8.0", LICENSE, BINARY_PACKAGE, URL)
